---
title: 'IP Intelligence API documentation'
sidebarTitle: 'IP Intelligence API'
icon: 'location-dot'
description: "Endpoint reference for Abstract's IP Intelligence API: base URL, request parameters, response fields for VPN, proxy, Tor, hosting, relay, mobile and abuse detection, error codes and code samples."
api: 'GET https://ip-intelligence.abstractapi.com/v1'
hideApiMarker: true
---

<Note>
  This page is the technical reference. For plans, the free tier and a live demo, see the [IP Intelligence API](https://www.abstractapi.com/api/ip-intelligence) product page.
</Note>
...
Your API key is your unique authentication key to be used to access Abstract's IP Intelligence API. Note that each of Abstract's APIs has a unique API key, so you will need different keys to access the IP Intelligence and Email Validation APIs, for example. You can send it as a query parameter or in an `Authorization` header: see [Authentication methods](#authentication-methods).
...
The IP Intelligence API supports multiple ways to authenticate your requests. You can use whichever method best fits your application.
...
```bash
curl 'https://ip-intelligence.abstractapi.com/v1/?api_key=YOUR_UNIQUE_API_KEY&ip_address=166.171.248.255'
```
...
When using header-based authentication, omit `api_key` from the query string. Only the `ip_address` parameter is needed.
...
In addition to GET requests, you can send POST requests to the IP Intelligence API. This is useful when you prefer to send parameters in the request body rather than the URL.

### POST with form-encoded data

Send both `api_key` and `ip_address` as form-encoded fields in the request body:
...
Authenticate via the `Authorization` header and send `ip_address` in a JSON request body:
...
When using the Bearer token with POST, the body only needs to contain the `ip_address` field. Send the `Content-Type: application/json` header with a JSON body, otherwise the body is read as form data and `ip_address` is not detected.
...
<ResponseField name="location.city_geoname_id" type="Integer">
  City's geoname ID.
</ResponseField>
...
<ResponseField name="location.region_geoname_id" type="Integer">
...
<ResponseField name="location.country_geoname_id" type="Integer">
  Country's geoname ID.
</ResponseField>

<ResponseField name="location.is_country_eu" type="Boolean">
...
<ResponseField name="location.continent_geoname_id" type="Integer">
...
<ResponseField name="currency.symbol" type="String">
