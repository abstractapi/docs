...
When using the Bearer token with POST, the body only needs to contain the `email` field. Send the `Content-Type: application/json` header with a JSON body, otherwise the body is read as form data and `email` is not detected.
...
<ResponseField name="suggested_correction" type="String or Null">
  If a typo has been detected, this returns a suggestion of the correct email
  (e.g., `johnsmith@gmial.com` => `johnsmith@gmail.com`). If no typo is detected,
  this is `null`.
</ResponseField>
